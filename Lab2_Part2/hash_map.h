...
template <typename K, typename V> void hash_map<K, V> :: hi_rehash()
{
    //1. determine the new table size
    size_t old_capacity = _capacity;
    if(old_capacity >= 2039)
    {
        _capacity = 2039;
    }
    else if(old_capacity < 209)
    {
        _capacity = 209;
    }
    else if(old_capacity >= 209 && old_capacity < 1021)
    {
        _capacity = 1021;
    }
    else
    ...
    hash_list<K,V> *newHead = new hash_list<K,V>[_capacity];

    //try to "migrate" all key,val pairs from original map to the new map
    for(size_t i = 0; i < old_capacity; i++)
    {
        _head[i].reset_iter();
        while(! _head[i].iter_at_end())
        {
            assert(_head[i].get_iter_value().has_value());
            K keykey = *(_head[i].get_iter_value().value().first);
            V valval = *(_head[i].get_iter_value().value().second);
            size_t ididx = _hash(keykey) % _capacity;
            newHead[ididx].insert(keykey, valval);
...
template <typename K, typename V> void hash_map<K, V> :: lo_rehash()
{
    //1. determine the new table size
    size_t old_capacity = _capacity;
    if(old_capacity > 2039)
    ...
    else if(_capacity > 209 && _capacity <= 1021)
    {
        _capacity = 209;
    }
    else
